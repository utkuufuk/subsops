...
function sendEmail(address, header, body) {
...
function updateSubscriber(request, response, type, oldState, newState) {
    return cors(request, response, () => {
        ...
        return db.doc('subscribers/' + id).get()
        .then(result => {
...
exports.notifySubscribers = functions.firestore.document('blogposts/{id}').onCreate((snap, _) => {
    const post = snap.data();
    db.collection("subscribers").where('state', '==', 'confirmed').get()
    .then(result => {
        const promises = [];
        result.forEach(result => {
            // send an email to each confirmed subscriber
            const sub = result.data();
            const body = 
                `Hey ${sub.name},<p>Check out my new blog post:<br>${post.url}<p>Have a nice day,` +
                `<p>Utku<p><a style="font-size: 9px" href=${functions.config().blog.website}` +
                `/unsubscribe/?name=${sub.name}&id=${sub.id}">unsubscribe</a>`;
            promises.push(sendEmail(sub.email, post.header, body));
            console.log(
                `New blog post notification email sent:` +
...
exports.subscribe = functions.https.onRequest((request, response) => {
    return cors(request, response, () => {
        // check if subscriber creation attempt is legal
        let sub = request.body;
        let valid = 
            sub.name !== undefined && sub.name.length > 0 && sub.name.length < 20 &&
            validator.validate(sub.email) && new RegExp(/^([a-zA-ZöçşığüÖÇŞİĞÜ]+)$/).test(sub.name);
        if (!valid) {
            // send a warning email and reject the request
            const body = 
                `Illegal subscriber creation attempt:\nEvent Date: ` +
                `${new Date().toISOString()} \nName: ${sub.name}\nEmail ${sub.email}`;
            return sendEmail(functions.config().admin.email, "Illegal Subscriber Creation", body)
            .then(result => {
        ...
        // accept the request & create a new subscriber in firestore
        sub.state = "unconfirmed";
        sub.createDate = new Date();
        sub.updateDate = new Date();
        return db.collection("subscribers").add(sub)
        .then(result => {
